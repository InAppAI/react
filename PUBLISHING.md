...
Create `packages/inapp-ai-react/CHANGELOG.md` if it doesn't exist:
...
- Bundles CSS
- Minifies code
- Outputs to `packages/inapp-ai-react/dist/`
...
npm pack
# Creates: inapp-ai-react-0.2.0.tgz

# View contents
tar -tzf inapp-ai-react-0.2.0.tgz
```
...
# In a test project
npm install /path/to/react/packages/inapp-ai-react/inapp-ai-react-0.2.0.tgz
...
```bash
# After building
du -sh packages/inapp-ai-react/dist
...
```bash
# Check for errors
npm run type-check --workspace=packages/inapp-ai-react

# Clean and rebuild
npm run clean --workspace=packages/inapp-ai-react
npm run build:package
```
