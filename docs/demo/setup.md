...
2. **npm** installed
3. **InAppAI Agent** (or access to the backend API)
...
4. Use the local emulator URL:
   ...
   VITE_API_BASE_URL=http://localhost:5001/inapp-ai-dev/us-central1/inappai/api
   ```
5. Create a test agent (see backend docs)
...
1. **Valid agent** configured in Firestore
...
2. Verify `VITE_AGENT_ID` and `VITE_API_BASE_URL` are set
...
2. Check that the agent exists in the backend
